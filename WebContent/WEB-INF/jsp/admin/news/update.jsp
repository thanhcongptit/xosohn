...
.ok {
	color: green
		...
		<%
			...
			String provincea = RequestUtil.getString(request, "province", "").toUpperCase().trim();
						...
						if(!oldImage.endsWith(fileItem.getName()) && !CommonUtil.isEmptyString(fileItem.getName())) {
							...
							if ("update".equals(action)) {
								File fileRemove = new File(
										request.getServletContext().getAttribute("FILES_DIR") + File.separator + oldImage);
								if(fileRemove.exists()) {
									fileRemove.delete();
								}
								
								File file1 = new File(
										request.getServletContext().getAttribute("FILES_DIR") + File.separator + folderString);
								...
								File file = new File(
										request.getServletContext().getAttribute("FILES_DIR") + File.separator  + fileName);
								fileItem.write(file);
							}
						} else {
							fileName = oldImage;
						...
						<table class="table-auto">
							...
							<tr>
								...
								<td><select name="province">
										<%
											for (LotteryCompany province : provinces) {
												if(!newsUpdate.getProvince().equalsIgnoreCase(province.getCompanyLink())) {	
										%>
										<option value="<%=province.getCompanyLink()%>"><%=province.getCompany()%></option>
										<%
												} else {
										%>
											<option selected="selected" value="<%=province.getCompanyLink()%>"><%=province.getCompany()%></option>
										<%}} %>
								</select></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<%
									...
									String filePath = request.getServletContext().getAttribute("FILES_DIR") + File.separator + oldImageFile;
							...
							<tr>
								...
								<td><a
									href="<%=request.getContextPath()%>/admin/news/index.htm"
