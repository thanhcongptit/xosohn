...
<div class="page table-item txt-heading mb20 icon-list">
	<h2>
		<span class="iconxs"><a href="/xsmt-xo-so-mien-trung.html">Xổ số miền trung</a></span> <span class="span">
		<span><a href="/${linkMTThu}">XSMT ${dayOfWeekMT}</a></span>
		<a href="/${linkMTDay}">XSMT ngày ${ddmmyyyyMT}</a></span>
	</h2>
	<c:if test="${lotterysMT!=null&&numSizeMT>0}">
		<c:set var="width" value="33%" />
		<c:if test="${numSizeMT>2}">
		...
		<table>
			...
			<tbody>
				<tr class="red">
					...
					<c:forTokens items="${lotterysMT.province}" delims="+"
						var="province">
		...
		<%  if(position != 1) {
		MetaGuide metaGuide = new MetaGuideDao().findByPosition(28);	
	
			
			String titleTemp = metaGuide.getTitle();
			
			String dateIndex = (String)request.getAttribute("ddmmyyyyMT");
	...
	<script type="application/ld+json">{
                  ...
                  "sameAs": "https://xosohn.com/xsmt-<%=version%>.html",
                  "keywords": [<%=keyword%>],
