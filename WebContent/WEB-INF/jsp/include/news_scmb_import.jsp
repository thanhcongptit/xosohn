...
<div class="list-news-item txt-heading">
	...
	<ul class="news">
		<c:forEach items="${listNews}" var="dream" >
			...
			<li><a href="<%= request.getContextPath() %>/soi-cau-mb/${dream.url}-${dream.id}.html">
					<div class="img">
						...
						<c:if test="${not empty dream.imageUrl}">
						    <img src="${pageContext.servletContext.contextPath}/images/news_scmb/${dream.imageUrl}" alt="" title="">
						</c:if>
	...
	<ul class="pagination">
		<li class="<c:if test="${1==iPage}">disabled</c:if>">
			<a href="<%= request.getContextPath() %>/soi-cau-mb/1.html">&laquo;</a>
		</li>
		...
		<c:forEach var="i" begin="1" end="${totalPage}" varStatus="loop">
			<c:if test="${index < iPage + 4 && index > iPage - 4}">
				<li class="<c:if test="${i==iPage}">active</c:if>"><a
					href="<%= request.getContextPath() %>/soi-cau-mb/${i}.html">${i}</a></li>
			</c:if>
			<c:set var="index" value="${index+1}" />
		</c:forEach>
		
		<li class="<c:if test="${totalPage==iPage}">disabled</c:if>">
			<a href="<%= request.getContextPath() %>/soi-cau-mb/${totalPage}.html">&raquo;</a>
		</li>
	</ul>
</div>