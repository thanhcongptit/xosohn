...
<div class="list-news-item txt-heading">
	<h2>Thống kê 2 số cuối giải đặc biệt</h2>
	<ul class="news">
		<c:forEach items="${listNews}" var="dream" >
		
			<li><a href="<%= request.getContextPath() %>/thong-ke-2-so-cuoi-giai-dac-biet/${dream.url}-${dream.id}.html">
					<div class="img">
						...
						<c:if test="${not empty dream.imageUrl}">
						    <img src="${pageContext.servletContext.contextPath}/images/news_db/${dream.imageUrl}" alt="" title="">
						</c:if>
	...
	<ul class="pagination">
		<li class="<c:if test="${1==iPage}">disabled</c:if>">
			<a href="<%= request.getContextPath() %>/thong-ke-2-so-cuoi-giai-dac-biet/1.html">&laquo;</a>
		</li>
		...
		<c:forEach var="i" begin="1" end="${totalPage}" varStatus="loop">
			<c:if test="${index < iPage + 4 && index > iPage - 4}">
				<li class="<c:if test="${i==iPage}">active</c:if>"><a
					href="<%= request.getContextPath() %>/thong-ke-2-so-cuoi-giai-dac-biet/${i}.html">${i}</a></li>
			</c:if>
			<c:set var="index" value="${index+1}" />
		</c:forEach>
		
		<li class="<c:if test="${totalPage==iPage}">disabled</c:if>">
			<a href="<%= request.getContextPath() %>/thong-ke-2-so-cuoi-giai-dac-biet/${totalPage}.html">&raquo;</a>
		</li>
	</ul>
</div>