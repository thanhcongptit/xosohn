...
//var fileConnector1 = 'connectors/' + lang + '/filemanager.' + lang;
var fileConnector = "test_connector.jsp";
	...
	$('#newfolder').unbind().click(function(){
		...
		var getFolderName = function(v, m){
			...
			if(fname != ''){
				...
				$.getJSON("https://xosohn.com/test_connector.jsp" + '?mode=addfolder&path=' + $('#currentpath').val() + '&name=' + foldername + '&time=' + d.getMilliseconds(), function(result){
					if(result['Code'] == 0){
...
var selectItem = function(data){
	...
	if(window.opener || window.tinyMCEPopup){
		...
		if($.urlParam('CKEditor')){
			...
			var tempUrl = url.replace("xosohn/", "");
			window.opener.CKEDITOR.tools.callFunction($.urlParam('CKEditorFuncNum'), tempUrl);
		} else {
	...
	var getNewName = function(v, m){
		...
		if(rname != ''){
			...
			$.ajax({
				type: 'GET',
				//url: connectString,
				url: "https://xosohn.com/test_connector.jsp"+ '?mode=rename&old=' + data['Path'] + '&new=' + givenName,
				dataType: 'json',
				async: false,
...
var deleteItem = function(data){
	...
	var doDelete = function(v, m){
		...
		$.ajax({
			type: 'GET',
			//url: connectString, 
			url: "https://xosohn.com/test_connector.jsp" + '?mode=delete&path=' + encodeURIComponent(data['Path']),
			dataType: 'json',
			async: false,
			success: function(result){
				if(result['Code'] == 0){
...
var getFileInfo = function(file){
	...
	var d = new Date(); // to prevent IE cache issues
	$.getJSON("https://xosohn.com/test_connector.jsp" + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
...
var getFolderInfo = function(path){
	...
	$.getJSON("https://xosohn.com/test_connector.jsp" + '?path=' + encodeURIComponent(path) + '&mode=getfolder&showThumbs=' + showThumbs + '&time=' + d.getMilliseconds() , function(data){
		var result = '';
...
$(function(){
	...
	$('#uploader').attr('action', "https://xosohn.com/test_connector.jsp");
